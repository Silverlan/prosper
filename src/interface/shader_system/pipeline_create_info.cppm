...
export {
	namespace prosper {
		...
		public:
			...
			bool AllowsDerivatives() const { return (m_createFlags & PipelineCreateFlags::AllowDerivativesBit) != PipelineCreateFlags::None; }

			bool AttachPushConstantRange(uint32_t in_offset, uint32_t in_size, ShaderStageFlags in_stages);

			/* Returns != UINT32_MAX if this pipeline is a derivative pipeline, or UINT32_MAX otherwise. */
			PipelineID GetBasePipelineId() const { return m_basePipelineId; }

			const PipelineCreateFlags &GetCreateFlags() const { return m_createFlags; }

			const std::vector<std::unique_ptr<DescriptorSetCreateInfo>> *GetDsCreateInfoItems() const { return &m_dsCreateInfoItems; }
			...
			bool GetShaderStageProperties(ShaderStage in_shader_stage, const ShaderModuleStageEntryPoint **out_opt_result_ptr_ptr) const;

			bool GetSpecializationConstants(ShaderStage in_shader_stage, const std::vector<SpecializationConstant> **out_opt_spec_constants_ptr, const unsigned char **out_opt_spec_constants_data_buffer_ptr) const;

			bool HasOptimizationsDisabled() const { return (m_createFlags & PipelineCreateFlags::DisableOptimizationBit) != PipelineCreateFlags::None; }
		...
		public:
			...
			bool AddSpecializationConstant(ShaderStage in_shader_stage, uint32_t in_constant_id, uint32_t in_n_data_bytes, const void *in_data_ptr) final
			{
				if(in_shader_stage != ShaderStage::Fragment && in_shader_stage != ShaderStage::Geometry && in_shader_stage != ShaderStage::TessellationControl && in_shader_stage != ShaderStage::TessellationEvaluation && in_shader_stage != ShaderStage::Vertex) {
					return false;
				}

				return BasePipelineCreateInfo::AddSpecializationConstant(in_shader_stage, in_constant_id, in_n_data_bytes, in_data_ptr);
			}
			...
			**/
			bool AddVertexBinding(uint32_t in_binding, VertexInputRate in_step_rate, uint32_t in_stride_in_bytes, const uint32_t &in_n_attributes, const VertexInputAttribute *in_attribute_ptrs, uint32_t in_divisor = 1);
			...
			**/
			void GetBlendingProperties(const float **out_opt_blend_constant_vec4_ptr_ptr, uint32_t *out_opt_n_blend_attachments_ptr) const;
			...
			*/
			...
			void GetDepthBoundsState(bool *out_opt_is_enabled_ptr, float *out_opt_min_depth_bounds_ptr, float *out_opt_max_depth_bounds_ptr) const;

			void GetDepthTestState(bool *out_opt_is_enabled_ptr, CompareOp *out_opt_compare_op_ptr) const;

			void GetEnabledDynamicStates(const DynamicState **out_dynamic_states_ptr_ptr, uint32_t *out_n_dynamic_states_ptr) const;

			void GetGraphicsPipelineProperties(uint32_t *out_opt_n_scissors_ptr, uint32_t *out_opt_n_viewports_ptr, uint32_t *out_opt_n_vertex_bindings_ptr, const IRenderPass **out_opt_renderpass_ptr_ptr, SubPassID *out_opt_subpass_id_ptr) const;

			void GetLogicOpState(bool *out_opt_is_enabled_ptr, LogicOp *out_opt_logic_op_ptr) const;

			void GetMultisamplingProperties(SampleCountFlags *out_opt_sample_count_ptr, const SampleMask **out_opt_sample_mask_ptr_ptr) const;
			...
			void GetRasterizationProperties(PolygonMode *out_opt_polygon_mode_ptr, CullModeFlags *out_opt_cull_mode_ptr, FrontFace *out_opt_front_face_ptr, float *out_opt_line_width_ptr) const;

			const IRenderPass *GetRenderPass() const { return m_renderpassPtr; }

			void GetSampleShadingState(bool *out_opt_is_enabled_ptr, float *out_opt_min_sample_shading_ptr) const;
			...
			**/
			void SetRasterizationProperties(PolygonMode in_polygon_mode, CullModeFlags in_cull_mode, FrontFace in_front_face, float in_line_width);
			...
			**/
			void SetScissorBoxProperties(uint32_t in_n_scissor_box, int32_t in_x, int32_t in_y, uint32_t in_width, uint32_t in_height);
			...
			**/
			void SetViewportProperties(uint32_t in_n_viewport, float in_origin_x, float in_origin_y, float in_width, float in_height, float in_min_depth, float in_max_depth);
			...
			*/
			void ToggleDepthBias(bool in_should_enable, float in_depth_bias_constant_factor, float in_depth_bias_clamp, float in_depth_bias_slope_factor);
			...
			*/
			void ToggleDepthBoundsTest(bool in_should_enable, float in_min_depth_bounds, float in_max_depth_bounds);
			...
			*/
			void ToggleDepthTest(bool in_should_enable, CompareOp in_compare_op);
			...
			**/
			void ToggleDynamicState(bool in_should_enable, const DynamicState &in_dynamic_state);
			void ToggleDynamicStates(bool in_should_enable, const DynamicState *in_dynamic_states_ptr, const uint32_t &in_n_dynamic_states);
			void ToggleDynamicStates(bool in_should_enable, const std::vector<DynamicState> &in_dynamic_states);
			...
			*/
			void ToggleLogicOp(bool in_should_enable, LogicOp in_logic_op);
		...
		private:
			...
			struct BlendingProperties {
				...
				BlendingProperties()
				{
					...
					channelWriteMask = ColorComponentFlags::ABit | ColorComponentFlags::BBit | ColorComponentFlags::GBit | ColorComponentFlags::RBit;
			...
			*/
			struct InternalScissorBox {
				...
				* @param in_height Height of the scissor box
				**/
				InternalScissorBox(int32_t in_x, int32_t in_y, uint32_t in_width, uint32_t in_height)
				{
					height = in_height;
					width = in_width;
					x = in_x;
			...
			*/
			typedef struct InternalViewport {
				...
				**/
				InternalViewport(float in_origin_x, float in_origin_y, float in_width, float in_height, float in_min_depth, float in_max_depth)
				{
					height = in_height;
			...
			*/
			typedef struct InternalVertexBinding {
				...
				**/
				InternalVertexBinding(uint32_t in_divisor, VertexInputRate in_rate, uint32_t in_stride_in_bytes)
				{
					divisor = in_divisor;
					rate = in_rate;
