	...
	namespace prosper {
		...
		class DLLPROSPER DescriptorArrayManager : public std::enable_shared_from_this<DescriptorArrayManager> {
		  public:
			...
			static std::shared_ptr<TDescriptorArrayManager> Create(IPrContext &context, const char *setName, const char *bindingName, ShaderStageFlags shaderStages);
			template<class TDescriptorArrayManager>
			static std::shared_ptr<TDescriptorArrayManager> Create(const std::shared_ptr<IDescriptorSetGroup> &matArrayDsg, uint32_t bindingIndex);
		  ...
		  protected:
			DescriptorArrayManager(const std::shared_ptr<IDescriptorSetGroup> &matArrayDsg, ArrayIndex maxArrayLayers, uint32_t bindingIndex = 0);
			std::optional<ArrayIndex> AddItem(const std::function<bool(IDescriptorSet &, ArrayIndex, uint32_t)> &fAddBinding);
			virtual void Initialize(IPrContext &context) {}
		  private:
			std::optional<ArrayIndex> PopFreeIndex();
			void PushFreeIndex(ArrayIndex index);
			std::shared_ptr<IDescriptorSetGroup> m_dsgArray = nullptr;
		...
		template<class TDescriptorArrayManager>
		std::shared_ptr<TDescriptorArrayManager> DescriptorArrayManager::Create(IPrContext &context, const char *setName, const char *bindingName, ShaderStageFlags shaderStages)
		{
			auto limits = context.GetPhysicalDeviceLimits();
			auto maxArrayLayers = limits.maxImageArrayLayers;
			auto createInfo = DescriptorSetCreateInfo::Create(setName);
			createInfo->AddBinding(bindingName, 0u, DescriptorType::CombinedImageSampler, maxArrayLayers, shaderStages);
			auto matArrayDsg = context.CreateDescriptorSetGroup(*createInfo);
			return Create<TDescriptorArrayManager>(matArrayDsg, 0u);
		}

		template<class TDescriptorArrayManager>
		std::shared_ptr<TDescriptorArrayManager> DescriptorArrayManager::Create(const std::shared_ptr<IDescriptorSetGroup> &matArrayDsg, uint32_t bindingIndex)
		{
			uint32_t arraySize;
