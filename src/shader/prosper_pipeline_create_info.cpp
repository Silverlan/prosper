...
bool prosper::BasePipelineCreateInfo::AttachPushConstantRange(uint32_t in_offset, uint32_t in_size, ShaderStageFlags in_stages)
{
	bool result = false;

	/* Retrieve pipeline's descriptor and add the specified push constant range */
	const auto new_descriptor = PushConstantRange(in_offset, in_size, in_stages);

	if(std::find(m_pushConstantRanges.begin(), m_pushConstantRanges.end(), new_descriptor) != m_pushConstantRanges.end()) {
		goto end;
	}
...
bool prosper::BasePipelineCreateInfo::GetShaderStageProperties(ShaderStage in_shader_stage, const ShaderModuleStageEntryPoint **out_opt_result_ptr_ptr) const
{
	bool result = false;
...
bool prosper::BasePipelineCreateInfo::GetSpecializationConstants(ShaderStage in_shader_stage, const std::vector<SpecializationConstant> **out_opt_spec_constants_ptr, const unsigned char **out_opt_spec_constants_data_buffer_ptr) const
{
	bool result = false;
	...
	if(sc_map_iterator != m_specializationConstantsMap.end()) {
		...
		if(out_opt_spec_constants_data_buffer_ptr != nullptr) {
			*out_opt_spec_constants_data_buffer_ptr = (m_specializationConstantsDataBuffer.size() > 0) ? &m_specializationConstantsDataBuffer.at(0) : nullptr;
		}
...
void prosper::BasePipelineCreateInfo::SetDescriptorSetCreateInfo(const std::vector<const DescriptorSetCreateInfo *> *in_ds_create_info_vec_ptr)
{
	...
	for(uint32_t n_descriptor_set = 0; n_descriptor_set < n_descriptor_sets; ++n_descriptor_set) {
		...
		if(reference_ds_create_info_ptr == nullptr) {
			m_dsCreateInfoItems.push_back(std::unique_ptr<DescriptorSetCreateInfo>());

			continue;
		}

		m_dsCreateInfoItems.push_back(std::unique_ptr<DescriptorSetCreateInfo>(new DescriptorSetCreateInfo(*reference_ds_create_info_ptr)));
	}
}

bool prosper::BasePipelineCreateInfo::AddSpecializationConstant(ShaderStage in_shader_stage, uint32_t in_constant_id, uint32_t in_n_data_bytes, const void *in_data_ptr)
{
	...
	m_specializationConstantsMap[in_shader_stage].push_back(SpecializationConstant(in_constant_id, in_n_data_bytes, data_buffer_size));

	m_specializationConstantsDataBuffer.resize(data_buffer_size + in_n_data_bytes);

	memcpy(&m_specializationConstantsDataBuffer.at(data_buffer_size), in_data_ptr, in_n_data_bytes);
  ...
  const std::vector<const DescriptorSetCreateInfo *> *in_opt_ds_create_info_vec_ptr)
{
	...
	InitShaderModules(in_n_shader_module_stage_entrypoints, in_shader_module_stage_entrypoint_ptrs);
}

void prosper::BasePipelineCreateInfo::InitShaderModules(uint32_t in_n_shader_module_stage_entrypoints, const ShaderModuleStageEntryPoint *in_shader_module_stage_entrypoint_ptrs)
{
...
prosper::GraphicsPipelineCreateInfo::GraphicsPipelineCreateInfo(const IRenderPass *in_renderpass_ptr, SubPassID in_subpass_id) : BasePipelineCreateInfo {}
{
	...
	memset(m_blendConstant.data(), 0, sizeof(m_blendConstant));
...
std::unique_ptr<prosper::GraphicsPipelineCreateInfo> prosper::GraphicsPipelineCreateInfo::Create(const PipelineCreateFlags &in_create_flags, const IRenderPass *in_renderpass_ptr, SubPassID in_subpass_id, const ShaderModuleStageEntryPoint &in_fragment_shader_stage_entrypoint_info,
  ...
  const ShaderModuleStageEntryPoint &in_vertex_shader_shader_stage_entrypoint_info, const GraphicsPipelineCreateInfo *in_opt_reference_pipeline_info_ptr, const PipelineID *in_opt_base_pipeline_id_ptr)
{
	...
	if(result_ptr != nullptr) {
		const ShaderModuleStageEntryPoint stages[] = {in_fragment_shader_stage_entrypoint_info, in_geometry_shader_stage_entrypoint_info, in_tess_control_shader_stage_entrypoint_info, in_tess_evaluation_shader_stage_entrypoint_info, in_vertex_shader_shader_stage_entrypoint_info};
		...
		result_ptr->init(in_create_flags, sizeof(stages) / sizeof(stages[0]), stages, in_opt_base_pipeline_id_ptr);
	}

	return result_ptr;
}

bool prosper::GraphicsPipelineCreateInfo::AddVertexBinding(uint32_t in_binding, VertexInputRate in_step_rate, uint32_t in_stride_in_bytes, const uint32_t &in_n_attributes, const VertexInputAttribute *in_attribute_ptrs, uint32_t in_divisor)
{
	bool result = false;
...
#ifdef _DEBUG
	{
		...
		for(const auto &current_binding : m_bindings) {
			for(const auto &current_attribute : current_binding.second.attributes) {
				for(uint32_t n_in_attribute = 0; n_in_attribute < in_n_attributes; ++n_in_attribute) {
	...
	* time. */
	InternalVertexBinding new_binding(in_divisor, in_step_rate, in_stride_in_bytes);
...
bool prosper::GraphicsPipelineCreateInfo::AreDepthWritesEnabled() const { return m_depthWritesEnabled; }

void prosper::GraphicsPipelineCreateInfo::GetBlendingProperties(const float **out_opt_blend_constant_vec4_ptr_ptr, uint32_t *out_opt_n_blend_attachments_ptr) const
{
...
bool prosper::GraphicsPipelineCreateInfo::GetColorBlendAttachmentProperties(SubPassAttachmentID in_attachment_id, bool *out_opt_blending_enabled_ptr, BlendOp *out_opt_blend_op_color_ptr, BlendOp *out_opt_blend_op_alpha_ptr, BlendFactor *out_opt_src_color_blend_factor_ptr,
...
void prosper::GraphicsPipelineCreateInfo::GetDepthBiasState(bool *out_opt_is_enabled_ptr, float *out_opt_depth_bias_constant_factor_ptr, float *out_opt_depth_bias_clamp_ptr, float *out_opt_depth_bias_slope_factor_ptr) const
{
...
void prosper::GraphicsPipelineCreateInfo::GetDepthBoundsState(bool *out_opt_is_enabled_ptr, float *out_opt_min_depth_bounds_ptr, float *out_opt_max_depth_bounds_ptr) const
{
	if(out_opt_is_enabled_ptr != nullptr) {
...
void prosper::GraphicsPipelineCreateInfo::GetDepthTestState(bool *out_opt_is_enabled_ptr, CompareOp *out_opt_compare_op_ptr) const
{
	if(out_opt_is_enabled_ptr != nullptr) {
...
void prosper::GraphicsPipelineCreateInfo::GetEnabledDynamicStates(const DynamicState **out_dynamic_states_ptr_ptr, uint32_t *out_n_dynamic_states_ptr) const
{
	*out_n_dynamic_states_ptr = static_cast<uint32_t>(m_enabledDynamicStates.size());
	*out_dynamic_states_ptr_ptr = ((m_enabledDynamicStates.size() > 0) ? &m_enabledDynamicStates.at(0) : nullptr);
}

void prosper::GraphicsPipelineCreateInfo::GetGraphicsPipelineProperties(uint32_t *out_opt_n_scissors_ptr, uint32_t *out_opt_n_viewports_ptr, uint32_t *out_opt_n_vertex_bindings_ptr, const IRenderPass **out_opt_renderpass_ptr_ptr, SubPassID *out_opt_subpass_id_ptr) const
{
...
void prosper::GraphicsPipelineCreateInfo::GetLogicOpState(bool *out_opt_is_enabled_ptr, LogicOp *out_opt_logic_op_ptr) const
{
	if(out_opt_is_enabled_ptr != nullptr) {
...
void prosper::GraphicsPipelineCreateInfo::GetMultisamplingProperties(SampleCountFlags *out_opt_sample_count_ptr, const SampleMask **out_opt_sample_mask_ptr_ptr) const
{
...
uint32_t prosper::GraphicsPipelineCreateInfo::GetDynamicScissorBoxesCount() const { return m_dynamicScissorBoxesCount; }

uint32_t prosper::GraphicsPipelineCreateInfo::GetDynamicViewportsCount() const { return m_dynamicViewportsCount; }

uint32_t prosper::GraphicsPipelineCreateInfo::GetScissorBoxesCount() const { return static_cast<uint32_t>(m_scissorBoxes.size()); }

uint32_t prosper::GraphicsPipelineCreateInfo::GetViewportCount() const { return static_cast<uint32_t>(m_viewports.size()); }

prosper::PrimitiveTopology prosper::GraphicsPipelineCreateInfo::GetPrimitiveTopology() const { return m_primitiveTopology; }

void prosper::GraphicsPipelineCreateInfo::GetRasterizationProperties(PolygonMode *out_opt_polygon_mode_ptr, CullModeFlags *out_opt_cull_mode_ptr, FrontFace *out_opt_front_face_ptr, float *out_opt_line_width_ptr) const
{
...
void prosper::GraphicsPipelineCreateInfo::GetSampleShadingState(bool *out_opt_is_enabled_ptr, float *out_opt_min_sample_shading_ptr) const
{
	if(out_opt_is_enabled_ptr != nullptr) {
...
bool prosper::GraphicsPipelineCreateInfo::GetScissorBoxProperties(uint32_t in_n_scissor_box, int32_t *out_opt_x_ptr, int32_t *out_opt_y_ptr, uint32_t *out_opt_width_ptr, uint32_t *out_opt_height_ptr) const
{
	bool result = false;
...
void prosper::GraphicsPipelineCreateInfo::GetStencilTestProperties(bool *out_opt_is_enabled_ptr, StencilOp *out_opt_front_stencil_fail_op_ptr, StencilOp *out_opt_front_stencil_pass_op_ptr, StencilOp *out_opt_front_stencil_depth_fail_op_ptr, CompareOp *out_opt_front_stencil_compare_op_ptr,
  uint32_t *out_opt_front_stencil_compare_mask_ptr, uint32_t *out_opt_front_stencil_write_mask_ptr, uint32_t *out_opt_front_stencil_reference_ptr, StencilOp *out_opt_back_stencil_fail_op_ptr, StencilOp *out_opt_back_stencil_pass_op_ptr, StencilOp *out_opt_back_stencil_depth_fail_op_ptr,
  CompareOp *out_opt_back_stencil_compare_op_ptr, uint32_t *out_opt_back_stencil_compare_mask_ptr, uint32_t *out_opt_back_stencil_write_mask_ptr, uint32_t *out_opt_back_stencil_reference_ptr) const
{
	if(out_opt_is_enabled_ptr != nullptr) {
...
bool prosper::GraphicsPipelineCreateInfo::GetVertexBindingProperties(uint32_t in_n_vertex_binding, uint32_t *out_opt_binding_ptr, uint32_t *out_opt_stride_ptr, VertexInputRate *out_opt_rate_ptr, uint32_t *out_opt_n_attributes_ptr, const VertexInputAttribute **out_opt_attributes_ptr_ptr,
  uint32_t *out_opt_divisor_ptr) const
{
	...
	if(out_opt_attributes_ptr_ptr != nullptr) {
		*out_opt_attributes_ptr_ptr = (binding_ptr->attributes.size() > 0) ? &binding_ptr->attributes.at(0) : nullptr;
	}
...
bool prosper::GraphicsPipelineCreateInfo::GetViewportProperties(uint32_t in_n_viewport, float *out_opt_origin_x_ptr, float *out_opt_origin_y_ptr, float *out_opt_width_ptr, float *out_opt_height_ptr, float *out_opt_min_depth_ptr, float *out_opt_max_depth_ptr) const
{
	bool result = false;
...
bool prosper::GraphicsPipelineCreateInfo::IsAlphaToCoverageEnabled() const { return m_alphaToCoverageEnabled; }

bool prosper::GraphicsPipelineCreateInfo::IsAlphaToOneEnabled() const { return m_alphaToOneEnabled; }

bool prosper::GraphicsPipelineCreateInfo::IsDepthClampEnabled() const { return m_depthClampEnabled; }

bool prosper::GraphicsPipelineCreateInfo::IsDepthClipEnabled() const { return m_depthClipEnabled; }

bool prosper::GraphicsPipelineCreateInfo::IsPrimitiveRestartEnabled() const { return m_primitiveRestartEnabled; }

bool prosper::GraphicsPipelineCreateInfo::IsRasterizerDiscardEnabled() const { return m_rasterizerDiscardEnabled; }

bool prosper::GraphicsPipelineCreateInfo::IsSampleMaskEnabled() const { return m_sampleMaskEnabled; }

void prosper::GraphicsPipelineCreateInfo::SetBlendingProperties(const float *in_blend_constant_vec4) { memcpy(m_blendConstant.data(), in_blend_constant_vec4, sizeof(m_blendConstant)); }

void prosper::GraphicsPipelineCreateInfo::SetColorBlendAttachmentProperties(SubPassAttachmentID in_attachment_id, bool in_blending_enabled, BlendOp in_blend_op_color, BlendOp in_blend_op_alpha, BlendFactor in_src_color_blend_factor, BlendFactor in_dst_color_blend_factor,
  BlendFactor in_src_alpha_blend_factor, BlendFactor in_dst_alpha_blend_factor, ColorComponentFlags in_channel_write_mask)
{
...
void prosper::GraphicsPipelineCreateInfo::SetMultisamplingProperties(SampleCountFlags in_sample_count, float in_min_sample_shading, const SampleMask in_sample_mask)
{
...
void prosper::GraphicsPipelineCreateInfo::SetDynamicScissorBoxesCount(uint32_t in_n_dynamic_scissor_boxes) { m_dynamicScissorBoxesCount = in_n_dynamic_scissor_boxes; }

void prosper::GraphicsPipelineCreateInfo::SetDynamicViewportCount(uint32_t in_n_dynamic_viewports) { m_dynamicViewportsCount = in_n_dynamic_viewports; }

void prosper::GraphicsPipelineCreateInfo::SetPrimitiveTopology(PrimitiveTopology in_primitive_topology) { m_primitiveTopology = in_primitive_topology; }

void prosper::GraphicsPipelineCreateInfo::SetRasterizationProperties(PolygonMode in_polygon_mode, CullModeFlags in_cull_mode, FrontFace in_front_face, float in_line_width)
{
...
void prosper::GraphicsPipelineCreateInfo::SetScissorBoxProperties(uint32_t in_n_scissor_box, int32_t in_x, int32_t in_y, uint32_t in_width, uint32_t in_height) { m_scissorBoxes[in_n_scissor_box] = InternalScissorBox(in_x, in_y, in_width, in_height); }

void prosper::GraphicsPipelineCreateInfo::SetStencilTestProperties(bool in_update_front_face_state, StencilOp in_stencil_fail_op, StencilOp in_stencil_pass_op, StencilOp in_stencil_depth_fail_op, CompareOp in_stencil_compare_op, uint32_t in_stencil_compare_mask,
  uint32_t in_stencil_write_mask, uint32_t in_stencil_reference)
{
	StencilOpState *const stencil_op_state_ptr = (in_update_front_face_state) ? &m_stencilStateFrontFace : &m_stencilStateBackFace;
...
void prosper::GraphicsPipelineCreateInfo::SetViewportProperties(uint32_t in_n_viewport, float in_origin_x, float in_origin_y, float in_width, float in_height, float in_min_depth, float in_max_depth)
{
	m_viewports[in_n_viewport] = InternalViewport(in_origin_x, in_origin_y, in_width, in_height, in_min_depth, in_max_depth);
}

void prosper::GraphicsPipelineCreateInfo::ToggleAlphaToCoverage(bool in_should_enable) { m_alphaToCoverageEnabled = in_should_enable; }

void prosper::GraphicsPipelineCreateInfo::ToggleAlphaToOne(bool in_should_enable) { m_alphaToOneEnabled = in_should_enable; }

void prosper::GraphicsPipelineCreateInfo::ToggleDepthBias(bool in_should_enable, float in_depth_bias_constant_factor, float in_depth_bias_clamp, float in_depth_bias_slope_factor)
{
...
void prosper::GraphicsPipelineCreateInfo::ToggleDepthBoundsTest(bool in_should_enable, float in_min_depth_bounds, float in_max_depth_bounds)
{
...
void prosper::GraphicsPipelineCreateInfo::ToggleDepthClamp(bool in_should_enable) { m_depthClampEnabled = in_should_enable; }

void prosper::GraphicsPipelineCreateInfo::ToggleDepthClip(bool in_should_enable) { m_depthClipEnabled = in_should_enable; }

void prosper::GraphicsPipelineCreateInfo::ToggleDepthTest(bool in_should_enable, CompareOp in_compare_op)
{
	m_depthTestEnabled = in_should_enable;
	m_depthTestCompareOp = in_compare_op;
}

void prosper::GraphicsPipelineCreateInfo::ToggleDepthWrites(bool in_should_enable) { m_depthWritesEnabled = in_should_enable; }

void prosper::GraphicsPipelineCreateInfo::ToggleDynamicState(bool in_should_enable, const DynamicState &in_dynamic_state)
{
	if(in_should_enable) {
		if(std::find(m_enabledDynamicStates.begin(), m_enabledDynamicStates.end(), in_dynamic_state) == m_enabledDynamicStates.end()) {
			m_enabledDynamicStates.push_back(in_dynamic_state);
		}
	}
	else {
		auto iterator = std::find(m_enabledDynamicStates.begin(), m_enabledDynamicStates.end(), in_dynamic_state);
...
void prosper::GraphicsPipelineCreateInfo::ToggleDynamicStates(bool in_should_enable, const DynamicState *in_dynamic_states_ptr, const uint32_t &in_n_dynamic_states)
{
	for(uint32_t n_dynamic_state = 0; n_dynamic_state < in_n_dynamic_states; ++n_dynamic_state) {
		const auto current_state = in_dynamic_states_ptr[n_dynamic_state];

		ToggleDynamicState(in_should_enable, current_state);
	}
}

void prosper::GraphicsPipelineCreateInfo::ToggleDynamicStates(bool in_should_enable, const std::vector<DynamicState> &in_dynamic_states)
{
...
void prosper::GraphicsPipelineCreateInfo::ToggleLogicOp(bool in_should_enable, LogicOp in_logic_op)
{
	m_logicOp = in_logic_op;
	m_logicOpEnabled = in_should_enable;
}

void prosper::GraphicsPipelineCreateInfo::TogglePrimitiveRestart(bool in_should_enable) { m_primitiveRestartEnabled = in_should_enable; }

void prosper::GraphicsPipelineCreateInfo::ToggleRasterizerDiscard(bool in_should_enable) { m_rasterizerDiscardEnabled = in_should_enable; }

void prosper::GraphicsPipelineCreateInfo::ToggleSampleMask(bool in_should_enable) { m_sampleMaskEnabled = in_should_enable; }

void prosper::GraphicsPipelineCreateInfo::ToggleSampleShading(bool in_should_enable) { m_sampleShadingEnabled = in_should_enable; }

void prosper::GraphicsPipelineCreateInfo::ToggleStencilTest(bool in_should_enable) { m_stencilTestEnabled = in_should_enable; }

///////

std::unique_ptr<prosper::ComputePipelineCreateInfo> prosper::ComputePipelineCreateInfo::Create(const PipelineCreateFlags &in_create_flags, const ShaderModuleStageEntryPoint &in_compute_shader_stage_entrypoint_info, const PipelineID *in_opt_base_pipeline_id_ptr)
{
	std::unique_ptr<ComputePipelineCreateInfo> result_ptr(nullptr, std::default_delete<ComputePipelineCreateInfo>());

	result_ptr.reset(new ComputePipelineCreateInfo());
